...
# #78 데모 리허설 실측: normal_deposit_terms clause_014 "은행이 이를 접수한
# 뒤..."를 모델이 "은행이 접수한 뒤..."로 인용해 목적어 대명사("이를")만
# 빠졌는데도 창작 인용으로 오판, 보험 조항 데모에서 "분석 실패"로 노출됨.
def test_목적어_대명사_생략은_같은_인용으로_인정():
    clause = ("제13조(사고·변경사항의 신고) ④ 제1항 및 제2항의 신고는 은행이 "
              "이를 접수한 뒤 전산입력 등 필요한 조치를 하는데 걸리는 합리적인 "
              "시간이 지나면 그 효력이 생기며")
    evidence = ("조항은 '은행이 접수한 뒤 전산입력 등 필요한 조치를 하는데 "
               "걸리는 합리적인 시간이 지나면 그 효력이 생기며'라고 규정합니다.")
    assert find_fabricated_quotes(evidence, clause) == []


def test_대명사_생략_완화가_실제_단어_추가까지_봐주지_않음():
    """대명사 생략 관용이 진짜 창작(새 단어 삽입)까지 통과시키면 안 된다 —
    #50 구멍 재발 방지 회귀."""
    clause = ("제13조(사고·변경사항의 신고) ④ 제1항 및 제2항의 신고는 은행이 "
              "이를 접수한 뒤 전산입력 등 필요한 조치를 하는데 걸리는 합리적인 "
              "시간이 지나면 그 효력이 생기며")
    evidence = ("조항은 '은행이 즉시 접수한 뒤 전산입력 등 필요한 조치를 "
               "하는데 걸리는 합리적인 시간이 지나면 그 효력이 생기며'라고 "
               "규정합니다.")
    fabricated = find_fabricated_quotes(evidence, clause)
    assert len(fabricated) == 1 and "즉시" in fabricated[0]


def test_대명사_어근은_무관한_단어를_훼손하지_않음():
    """'이를'/'그것을'은 완전한 조사결합형일 때만 지워야 한다 — '이'/'그' 어근을
    지우면 '이 조항'처럼 뒤 명사를 꾸미는 관형사까지 사라져 무관한 문장이
    우연히 일치할 위험이 있다."""
    clause = "이 조항은 계약 갱신 절차를 정한다."
    evidence = "조항은 '그 조항은 계약 갱신 신청을 정한다'고 명시합니다."
    fabricated = find_fabricated_quotes(evidence, clause)
    assert fabricated == ["그 조항은 계약 갱신 신청을 정한다"]
